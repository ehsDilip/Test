...
$request['TransactionDetail'] = array('CustomerTransactionId' => '*** Close with Documents Request using PHP ***');
...
$request['ReprintCloseDate'] = date('c');
...
try {
	...
	$response = $client ->closeWithDocuments($request);

    if ($response -> HighestSeverity != 'FAILURE' && $response -> HighestSeverity != 'ERROR'){
    	if(array_key_exists("Documents", $response) && is_array ($response->Documents)){ 
			foreach($response->Documents as $closeDocument){
				getDocument($closeDocument);
			}
		}elseif(array_key_exists("Documents", $response)){
			getDocument($response->Documents);
		}
	}
